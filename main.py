...
config_flags.DEFINE_config_file('agent', 'agents/van.py', lock_config=False)
...
def main(_):
    ...
    run = setup_wandb(project='ICML_Flowmatching_0130', group=task_id, name=f"{base_env}_n={FLAGS.horizon_length}")
    ...
    def process_train_dataset(ds):
        ...
        if FLAGS.sparse:
            ...
            print("reward == 0 개수:", int(num_zero))
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + config["bcflowstep"] +1)):
        log_step += 1
        ...
        if log_step<config["bcflowstep"]:
            agent, offline_info = agent.bc_flow_update(batch)
        elif log_step==config["bcflowstep"] and log_step!=0:
            agent.bctoactor()
        else :
        ...
        if i == FLAGS.offline_steps - 1 or \
            (FLAGS.eval_interval != 0 and i % FLAGS.eval_interval == 0) and i>config["bcflowstep"]:
            ...
            if FLAGS.video_episodes>0:
                ...
                for i, video in enumerate(renders):
                    path = f"videos/{base_env}/task{task_id}_eval_ep_{log_step}_{i}.mp4"
                    imageio.mimsave(
                        path,
                        video,
